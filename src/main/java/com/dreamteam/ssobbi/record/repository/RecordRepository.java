...
public interface RecordRepository extends JpaRepository<Record,Long> {
    ...
    @Query("select r from Record r left join fetch r.consumptions where r.date = :date and r.user.id = :userId")
    Optional<Record> findByDateAndUserWithConsumption(LocalDate date, Long userId);

    @Query("select r from Record r left join fetch r.consumptions where r.date between :startDate and :endDate and r.user.id = :userId")
    ...
    ArrayList<Record> findByUser(User user);
}
